...
class OrchestratorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Setup test fixtures
    initial_positions = {RVO::Vector3(0.0f, 0.0f, 10.0f),
                         RVO::Vector3(1.0f, 0.0f, 10.0f),
                         RVO::Vector3(2.0f, 0.0f, 10.0f)};
    orchestrator =
        new Orchestrator(initial_positions, 0.008f, 3.0f, 20, 0.8f, 0.4f, 0.5f);
  }

  void TearDown() override { delete orchestrator; }
...
TEST_F(OrchestratorTest, SetPositionsAndVelocities) {
  std::vector<RVO::Vector3> new_positions = {RVO::Vector3(5.0f, 5.0f, 10.0f),
                                             RVO::Vector3(6.0f, 5.0f, 10.0f),
                                             RVO::Vector3(7.0f, 5.0f, 10.0f)};
  std::vector<RVO::Vector3> new_velocities = {RVO::Vector3(0.1f, 0.0f, 0.0f),
                                              RVO::Vector3(0.1f, 0.0f, 0.0f),
                                              RVO::Vector3(0.1f, 0.0f, 0.0f)};

  orchestrator->SetWebotDronePositionsAndVelocities(new_positions,
                                                    new_velocities);
...
TEST_F(OrchestratorTest, SetGoals) {
  std::vector<RVO::Vector3> goals = {RVO::Vector3(10.0f, 10.0f, 10.0f),
...
TEST_F(OrchestratorTest, ReachedGoalsReturnsTrueWhenClose) {
  std::vector<RVO::Vector3> goals = {
      ...
      RVO::Vector3(1.1f, 0.1f, 10.0f), RVO::Vector3(2.1f, 0.1f, 10.0f)};
  ...
  orchestrator->SetWebotDronePositionsAndVelocities(close_positions,
                                                    zero_velocities);

  EXPECT_TRUE(orchestrator->ReachedGoals());
}

TEST_F(OrchestratorTest, ReachedGoalsReturnsFalseWhenFar) {
  std::vector<RVO::Vector3> goals = {
      RVO::Vector3(10.0f, 10.0f, 10.0f),  // Far from initial position
      RVO::Vector3(11.0f, 10.0f, 10.0f), RVO::Vector3(12.0f, 10.0f, 10.0f)};

  orchestrator->SetWebotDroneGoals(goals);

  EXPECT_FALSE(orchestrator->ReachedGoals());
}

// Test GetNewVelocities
TEST_F(OrchestratorTest, GetNewVelocitiesReturnsValidVelocities) {
  std::vector<RVO::Vector3> goals = {RVO::Vector3(1.0f, 0.0f, 10.0f),
                                     RVO::Vector3(2.0f, 0.0f, 10.0f),
...
TEST_F(OrchestratorTest, SingleDroneInitialization) {
  std::vector<RVO::Vector3> single_pos = {RVO::Vector3(0.0f, 0.0f, 10.0f)};
...
TEST_F(OrchestratorTest, GetDroneVelocity) {
  ...
  std::vector<RVO::Vector3> velocities = {RVO::Vector3(0.2f, 0.1f, 0.0f),
...
TEST_F(OrchestratorTest, MultipleIterationsConvergeToGoals) {
  std::vector<RVO::Vector3> goals = {RVO::Vector3(5.0f, 0.0f, 10.0f),
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    orchestrator->SetWebotDronePositionsAndVelocities(current_positions,
                                                      current_velocities);
    std::vector<RVO::Vector3> new_velocities = orchestrator->GetNewVelocities();
    ...
    for (size_t j = 0; j < 3; ++j) {
      updated_positions.push_back(current_positions[j] +
                                  new_velocities[j] * 0.008f);
    }

    orchestrator->SetWebotDronePositionsAndVelocities(updated_positions,
                                                      new_velocities);
  }
