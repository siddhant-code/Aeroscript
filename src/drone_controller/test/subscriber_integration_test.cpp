/**
 * @file subscriber_integration_test.cpp
...
TEST_CASE_METHOD(SubscriberIntegrationTestFixture,
  ...
  auto vel_subscriber =
      tester_node->create_subscription<geometry_msgs::msg::Twist>(
          "Mavic_2_PRO_1/cmd_vel", 10,
          [&received_velocities,
           this](const geometry_msgs::msg::Twist::SharedPtr msg) {
            received_velocities.push_back(*msg);
            RCLCPP_INFO(this->tester_node->get_logger(),
                        "Received velocity: x=%.3f, y=%.3f, z=%.3f",
                        msg->linear.x, msg->linear.y, msg->linear.z);
          });

  // Publish position data that the controller subscribes to
  auto pos_publisher =
      tester_node->create_publisher<geometry_msgs::msg::PointStamped>(
          "Mavic_2_PRO_1/gps", 10);

  auto vel_publisher =
      tester_node->create_publisher<geometry_msgs::msg::Vector3>(
          "Mavic_2_PRO_1/gps/speed_vector", 10);

  // Wait for publishers to be ready
  rclcpp::sleep_for(500ms);
  rclcpp::spin_some(tester_node);

  // Wait for mavic_controller node to initialize (give it time even if it will
  // crash)
  ...
  // Publish initial messages to wake up the mavic_controller
  for (int i = 0; i < 10; ++i) {
    pos_msg.header.stamp = tester_node->now();
...
TEST_CASE_METHOD(SubscriberIntegrationTestFixture,
                 "Test multiple drone coordination", "[integration]") {
  const int num_drones = 3;
  std::vector<std::vector<geometry_msgs::msg::Twist>> received_velocities(
      num_drones);
  ...
  for (int i = 0; i < num_drones; ++i) {
    ...
    vel_subs.push_back(
        tester_node->create_subscription<geometry_msgs::msg::Twist>(
            drone_name + "/cmd_vel", 10,
            [&received_velocities, i,
             this](const geometry_msgs::msg::Twist::SharedPtr msg) {
              received_velocities[i].push_back(*msg);
              RCLCPP_INFO(this->tester_node->get_logger(),
                          "Drone %d: Received velocity: x=%.3f, y=%.3f, z=%.3f",
    ...
    vel_pubs.push_back(
        tester_node->create_publisher<geometry_msgs::msg::Vector3>(
            drone_name + "/gps/speed_vector", 10));
  }
  ...
  if (!any_received) {
    RCLCPP_WARN(tester_node->get_logger(),
                "No velocity commands received for any drone. Listener node "
                "may have crashed.");
    RCLCPP_WARN(tester_node->get_logger(),
                "Check if CSV file exists at: "
                "src/drone_controller/src/letters_AZ_cad.csv");
...
TEST_CASE_METHOD(SubscriberIntegrationTestFixture,
                 "Test velocity command frequency", "[integration]") {
  ...
  if (received_times.size() > 1) {
    auto total_duration =
        (received_times.back() - received_times.front()).seconds();
